...
class CustomerServicePactTest {
    ...
    fun createPact(builder: PactDslWithProvider): RequestResponsePact {
        return builder
                ...
                .uponReceiving("get request on /$segment")
                .path("/$segment")
                .method("GET")
                .willRespondWith()
                ...
                .uponReceiving("get request on /$segment/555")
                .path("/$segment/555")
                .method("GET")
                .willRespondWith()
                .status(HttpStatus.SC_OK)
                ...
                .uponReceiving("get request on /$segment/0")
                .path("/$segment/0")
                .method("GET")
                .willRespondWith()
                .status(HttpStatus.SC_NOT_FOUND)
                .given("there is no customer with invalid id a")
                .uponReceiving("get request on /$segment/a")
                .path("/$segment/a")
                .method("GET")
                .willRespondWith()
    ...
    internal fun test(mockServer: MockServer) {
        val allCustomersResponse = Request.Get(mockServer.getUrl() + "/$segment").execute().returnResponse()
        assertThat(allCustomersResponse.statusLine.statusCode).isEqualTo(200)

        val singleCustomerResponse = Request.Get(mockServer.getUrl() + "/$segment/555").execute().returnResponse()
        assertThat(singleCustomerResponse.statusLine.statusCode).isEqualTo(200)

        val noCustomerResponse = Request.Get(mockServer.getUrl() + "/$segment/0").execute().returnResponse()
        assertThat(noCustomerResponse.statusLine.statusCode).isEqualTo(404)

        val badRequestResponse = Request.Get(mockServer.getUrl() + "/$segment/a").execute().returnResponse()
